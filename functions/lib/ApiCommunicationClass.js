...
class ApiCommunicationClass extends CheckInformationClass {
  ...
  // OpenStreetMap API.
  async geoLocateUser(lon, lat) {
    const verifiedCoords = this.checkCoordinates(lon, lat);
    const request = "https://nominatim.openstreetmap.org/reverse?format=jsonv2&lon=" + verifiedCoords.lon + "&lat=" + verifiedCoords.lat + "&zoom=18";
